...
/**
 * A tus-style resumable endpoint that also tracks records — the gap the
 * records-free {@see ResumableUploadHandler} leaves open.
 *
 * It registers a draft {@see \PHPdot\Filesystem\ManagedFiles\FileRecord} at POST
 * (capturing the declared name) and finalizes it on the PATCH that completes the
 * upload. Incomplete uploads stay drafts and expire, swept by
 * {@see Files::purge}. The byte transport is the unchanged
 * {@see UploadManagerInterface}.
 */
final class ManagedResumableUploadHandler implements RequestHandlerInterface
{
    ...
    private function create(ServerRequestInterface $request): ResponseInterface
    {
        ...
        $this->files->registerUpload($session->path, new FileContext(originalName: $filename));
    ...
    private function writeChunk(ServerRequestInterface $request): ResponseInterface
    {
        ...
        if ($result->complete) {
            // Capture the path before complete() retires the session, then
            // finalize the draft record from the written bytes.
            $path = $this->uploads->status($sessionId)->path;
            $this->uploads->complete($sessionId);
            $this->files->finalizeUpload($path);
        }
